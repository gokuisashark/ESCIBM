...
public class Text extends Captcha {
    ...
    public enum TextOptions {//options.
        UPPERCASE_ONLY,
        LOWERCASE_ONLY,
        NUMBERS_ONLY,
        LETTERS_ONLY,
    ...
    public Text(int wordLength, TextOptions opt) {
        new Text(0, 0, wordLength, opt);
    }


    public Text(int width, int height, int wordLength, TextOptions opt) {
        setHeight(height);
        setWidth(width);
        this.options = opt;
    ...
    protected Bitmap image() {
        ...
        this.answer = "";
        for (int i = 0; i < this.wordLength; i++) { //determine characters required.
            char ch = ' ';
            switch (options) {
                case UPPERCASE_ONLY:
            ...
            cab.append(ch);
            this.answer += ch; //keep appending to characters to add in.
        }
